b// 07/29/22
// ==============================================================================
// How Do I add These two events to the dom
/* 
HEADS UP: Part of the challenge is to figure out
          how you are going to validate that your
          code works. Feel free to ask, but attempt
          to come up with a way to test if your code
          works on your own or w/ a partner first. 
<body>
  <h1>Hello World</h1>
  <div id="uniqueValue">
    <p class="bold">lorem ipsum</p>
  </div>
</body>

*/

// Click on the div with id of unique value that 
// appends another p tag with textContent of "Hello world"

// Adds keydown for the body that changes the color of 
// the font to blue if you press 'u',
// black if you press 'b',
// red if you press 'r'

//
// ==============================================================================

// ==============================================================================
// Navigate to this link: https://github.com/public-apis/public-apis
// Pick an api without any authentication
// Console log the data with fetch 
// ==============================================================================



// 07/22/22
// ==============================================================================
// Write down the 7 primitive data types, with an example. I.E. `String:"example"
// ==============================================================================
/* 
  WRITE OUT YOUR DEFINITIONS HERE! 
*/
// String = a word or group of words;
// Boolean = true or false values;
// Null = the absence of a value;
// bigInt() = used to display large numbers
// undefined = no value has been assigned, or nothing was returned
// symbol = %#!
// number = 123





<<<<<<< HEAD






// Write out your definition for what a function and a callback is. Give examples

// --------------------ANSWER-------------------------
// A function is a block of code that does a specific task.  
// A callback is a function that is passed as an argument.


=======
// Write out your definition for what a function and a callback is. Give examples 
>>>>>>> b8cf114495ad713b746c0f501de2a64c6716ae69
// of a named function, arrow function, and an anonymous arrow function


// ==============================================================================
/* 
  WRITE OUT YOUR DEFINITIONS HERE! 
  WRITE OUT CODE EXAMPLES BELOW
*/

function foo() {

}

const x = () => {

}











// Your Definition of an array, forEach, map, find, and filter. 
// --------------------ANSWER------------------------
// An array is used to store values.

// The forEach() method calls a function for each element in an array.

// The map method transforms every element in an array and returns a new array.

// The find method automatically iterates through an array, and calls the callback function on each value until the first element that satisfies the defined function is met.

// The filter method iterated through an array, and calls a callback function on every element in the array, even after the function is satisfied.

// (For the iterators please specify the return values of function & callback)
// ----------------------ANSWER-------------------------- 

// forEach()'s return is defaulted to undefined.

// .map() returns a new array.

// .find() returns the first matching element.

// .filter() returns all the elements in the array, even after the defined function is satisfied.

// Give examples of each iterator. I.E.
/*
  forEach returns undefined and the return of the callback does not matter.
*/
arr.forEach((el) => console.log(el))
// ==============================================================================
/* 
  WRITE OUT YOUR DEFINITIONS HERE! 
  WRITE OUT CODE EXAMPLES BELOW

  const array1 = [5, 12, 8, 130, 44];

const found = array1.find(element => element > 10);

const map1 = array1.map(x => x * 2);

const words = ['spray', 'limit', 'elite', 'exuberant', 'destruction', 'present'];

const result = words.filter(word => word.length > 6);
*/













// Your definition of an object. Write a complex object that describes a home. I.E
const home = { 
  address: { 
    streetName:"" , 
    streetNum: "69TH", 
    zip: 34221
  }, 
  rooms: {  
    numberOfBedRooms: 4,
    numberOfBathrooms: 2
  },
  hasGarage: true,
}
// ==============================================================================
/* 
  WRITE OUT YOUR DEFINITIONS HERE! 

  An object is used to store values.
  
  WRITE OUT CODE EXAMPLES BELOW
*/

  const obj = {
    numbers: 123,
    string: "string",
    boolean: true,
  }













// Write out 4 selectors that you know of. Using js Create a unordered list with 4 list items.
// ==============================================================================
/* 
  WRITE OUT YOUR DEFINITIONS HERE! 
  
h1, p, li, ol,

  WRITE OUT CODE EXAMPLES BELOW
*/

<ul>
  <li>THIS</li>
  <li>IS</li>
  <li>A</li>
  <li>LIST</li>
</ul>
 











